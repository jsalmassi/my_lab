...
class NetworkEnvelope:
    ...
    def parse(cls, s, testnet=False):
        ...
        if checksum != hash256(payload)[:4]:
            raise RuntimeError('checksum is not right')
...
class SimpleNode:
    ...
    def handshake(self):
        ...
        self.send(version)  # (2)
        self.wait_for(VerAckMessage)  # (3)
        """ verack_received = False
        version_received = False
        while not verack_received and not version_received:  # (3)
            message = self.wait_for(VersionMessage, VerAckMessage)  # (4)
    ...
    def send(self, message):  # <1>
        ...
        if self.logging:
            ...
            # log = f"[{timestamp}] SENDING: {envelope}"
            log = f"[{now}] SENDING: {envelope}"
            print(log)
    ...
    def read(self):  # <2>
        ...
        if self.logging:
            now = datetime.now(timezone.utc).isoformat()
            #timestamp = time.strftime("%H:%M:%S", time.gmtime())
            # log = f"[{timestamp}] RECEIVING: {envelope}"
            log = f"[{now}] RECEIVING: {envelope}"
            print(log)
...
class SimpleNodeTest(TestCase):
    ...
    def test_handshake(self):
        ...
        # the above domain is not working, using IP directly that I got from my_get_testnet_peers.py
        node = SimpleNode('195.201.246.33', testnet=True, logging=True) #195.201.246.33
        node.handshake()

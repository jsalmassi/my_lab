...
class MerkleTreeTest(TestCase):
    ...
    def test_populate_tree_1(self):
        ...
        #tree.populate_tree([1] * 32, hashes)
...
class MerkleBlock:
    ...
    def is_valid(self):
        ...
        reversed_hashes = [h[::-1] for h in self.hashes]    
        tree = MerkleTree(len(reversed_hashes))
        for i, (flag_bit, hash) in enumerate(zip(flag_bits, reversed_hashes)):
            if flag_bit:
                tree.nodes[0][i] = hash
